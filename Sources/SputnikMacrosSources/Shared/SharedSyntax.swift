...
enum SharedSyntax {
    ...
    static func getMapDataSyntax(declarations: [VariableDeclSyntax]) -> String {
        let functionBody = declarations.map { declaration -> String in
            ...
            let optionalMidfix = type.isOptional ? "?" : ""
                ...
                if type.isArray {
                    ...
                    if let dataElements = data[\"\(dataName)\"] as? [Any] {
                        ...
                        for index in 0..<dataElements.count {
                            ...
                            self.\(variableName)\(optionalMidfix)[index].mapData(data)
                        }
                    }
                    """
                    ...
                    if let data = data[\"\(dataName)\"] {
                            self.\(variableName)\(optionalMidfix).mapData(data)
                        }
                    """
                }
            } else {
                                         ...
                                         in context: MacroExpansionContext) throws -> [DeclSyntax]
    {
        ...
        let variableDefinitons = variables.map { variable -> String in
            """
        ...
        let variablesDeclarationString = variableDefinitons.joined(separator: "\n") + "\n"
        ...
        let totalProperies = variablesPropertyNames + storedPropertyNames
        let totalParameters = parameters + variablesParameters

        let initializer = """
        init(\(totalParameters.joined(separator: ",\n      "))) {
            \(totalProperies.map { "self.\($0) = \($0)" }.joined(separator: "\n    "))
        }
    ...
    static func getOperationExtensionSharedSyntax(of node: AttributeSyntax,
                                                  ...
                                                  sateliteType: OperationType) throws -> String
    {
        let arguments = node.arguments?.as(LabeledExprListSyntax.self)
        let variableMappingArgument = arguments?.first(where: { $0.label?.text == "variableMapping" })?.expression ?? ""
        let nameArgument = arguments?.first(where: { $0.label?.text == "name" })?.expression.description ?? "nil"
        let mappingType = extractVariableMappingType(from: variableMappingArgument)
        var protocolConformances = [sateliteType.protocolConformance]
        ...
        extension \(type): \(protocolConformancesSyntax) {
            ...
            mutating func setParialErrors(_ errors: [Dependencies.ResponsePartialError]) {
    ...
    private static func extractVariableMappingType(from expr: ExprSyntax) -> VariableMapping {
                                         ...
                                         in context: MacroExpansionContext) -> [String]
    {
        ...
        for member in declaration.memberBlock.members {
            if let nestedStruct = member.decl.as(StructDeclSyntax.self) {
                let hasAttribute = nestedStruct.attributes.contains { attr in
                    attr.as(AttributeSyntax.self)?.attributeName.trimmedDescription == "QuerySatelite"
                }
                if !hasAttribute {
                    context.throwError(error: .querySateliteApplyableOnly, of: node)
                }
