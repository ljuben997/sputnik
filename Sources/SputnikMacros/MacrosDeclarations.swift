...
/// setSelections() function. This function is used for you to define exatcly which values from the QuerySatelite you want the response to return.
/// - Parameter schemaTypeName: Use this when your struct name is not the same as the name in the schema definitiona, but only if the struct.
...
///     struct MeditaType {}
/// }
/// But in the schema definiton the object's type name is Type then this won't work when mapping variables. It must be rewritten as:
/// @QueryOperation
/// struct Media {
///     @Variable var media MediaType
///     @QuerySatelite(schemaTypeName: "type")
///     struct MeditaType {}
/// }
@attached(member, names: arbitrary)
@attached(extension, conformances: GraphQLSatelite, names: arbitrary)
public macro QuerySatelite(schemaTypeName: String? = nil) = #externalMacro(module: "SputnikMacrosSources", type: "QuerySateliteMacro")

/// QueryOperation macro. Use this to define the main GraphQLQuery. Just like in the @QuerySatelite macro this also adds the function setSelections().
...
/// QueryMacro macro. Use this to define the main GraphQLMutation. Just like in the @QuerySatelite macro this also adds the function setSelections().
...
/// - Parameter transfomer: A transformer type for initializing the scalar from the response and serializing to for a request. By default we use
