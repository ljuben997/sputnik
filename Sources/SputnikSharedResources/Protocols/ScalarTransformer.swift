...
/// class called "DateScalarTransformer". Please have a look at it to see how to use this properly when you are trying to build your own custom scalar transfomer.
public protocol ScalarTransformer {
    associatedtype T
    /// Initialize the value from a string to a sclar object. If it can't be initialized from a string then simply return a nil.
    static func initialize(stringValue: String) -> T?
    /// Initialize the value from a int to a sclar object. If it can't be initialized from an int then simply return a nil.
    static func initialize(intValue: Int) -> T?
    /// Initialize the value from a double value to a sclar object. If it can't be initialized from a double value then simply return a nil.
    static func initialize(doubleValue: Double) -> T?
    /// Initialize the value from a boolean value to a sclar object. If it can't be initialized from a boolean then simply return a nil.
